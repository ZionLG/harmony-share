...
const ProductSearch = ({ maxResults = 5 }: SearchProps) => {
  ...
  const getPublicPlaylists = api.spotify.getSong.useQuery(
    ...
    <div className="relative flex flex-col">
      <div
        className={`flex border ${
          (getPublicPlaylists.data?.tracks.length ?? 0) > 0
            ? " rounded-t-md"
            : " rounded-md"
      ...
      >
        ...
        <input
          className=" rounded-lg border-none px-5 "
          type="text"
          value={searchTerm}
      ...
      {(getPublicPlaylists.data?.tracks.length ?? 0) > 0 && (
        <div className="absolute top-full z-50 w-full rounded-b border-x border-b  p-2">
          {getPublicPlaylists.data?.tracks.map((v) => formatResult(v))}
        </div>
      )}
    </div>
  );
