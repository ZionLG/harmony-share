...
export default function PlaylistPage() {
  ...
  const { mutate, isLoading } = api.playlist.toggleLikePlaylist.useMutation({
    async onMutate() {
      ...
      const optimisticPlaylistLike = {
        ...prevData,
      };
      if (optimisticPlaylistLike.playlist && session.data) {
        if (
  ...
  return (
    <main className="gap flex min-h-screen flex-col p-5">
      ...
      <div className="flex items-center gap-5">
        ...
        <div
          className={getPlaylist.data.playlist.likes.length > 0 ? "group" : ""}
        >
          <HeartIcon
            onClick={() =>
              !isLoading && mutate({ playlistId: getPlaylist.data.playlist.id })
            }
            fill={getPlaylist.data.playlist.likes.length > 0 ? "red" : ""}
            className="cursor-pointer text-lg group-hover:hidden "
          >
            {getPlaylist.data.playlist.likes.length > 0 ? "Liked" : "Like"}
          </HeartIcon>
          <HeartCrackIcon
            onClick={() =>
              !isLoading && mutate({ playlistId: getPlaylist.data.playlist.id })
